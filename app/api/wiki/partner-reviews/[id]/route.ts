...
// Soft-delete (archive). Any attendee OR steward.
export async function DELETE(
  ...
  if (!meeting) return Response.json({ error: "Not found" }, { status: 404 });
  if (meeting.archivedAt) return Response.json({ error: "Already archived" }, { status: 409 });

  const isAttendee = meeting.attendees.some((a) => a.id === userId);
  const steward = isAttendee ? false : await isWikiSteward(userId);
  if (!isAttendee && !steward) {
    return Response.json({ error: "Forbidden" }, { status: 403 });
  }

  await prisma.wikiPartnerReviewMeeting.update({
    where: { id: meeting.id },
