...
export default function PlayWorkbench({ instanceId, instanceName, scenarioName, template, initialInputs, siblings, headId, attachedPitstop }: Props) {
        ...
        <div className="px-5 py-4 border-b border-stone-200 sticky top-0 bg-white z-10">
          ...
          {siblings && (
            <div className="mt-3 flex items-center gap-1 flex-wrap">
              ...
              <Link
                href={`/models/${headId}/compare`}
                className="text-xs px-2 py-0.5 rounded text-emerald-700 hover:bg-emerald-50"
              >
                Compare →
              </Link>
            </div>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mt-8 space-y-8">
          {template.outputs.filter(o => o.kind === "series").map(o => (
            <SeriesChart key={o.key} output={o} values={result.values} template={template} />
          ))}
...
function KpiRow({ outputs, values }: { outputs: ModelOutput[]; values: Record<string, NodeValue> }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {outputs.map(o => {
        ...
        return (
          <div key={o.key} className="bg-white border border-stone-200 rounded-xl p-4">
            <div className="text-xs text-stone-500">{o.label}</div>
...
function SeriesChart({ output, values, template }: { output: ModelOutput; values: Record<string, NodeValue>; template: ModelTemplate }) {
